...
- (void)didInitialize {
    ...
    self.imagePosition = JXButtonImagePositionLeft;
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    switch (self.imagePosition) {
        case JXButtonImagePositionTop:
        case JXButtonImagePositionBottom: {
        ...
        case JXButtonImagePositionLeft:
        case JXButtonImagePositionRight: {
            // 图片和文字水平排版时，高度以文字或图片的最大高度为最终高度
            // 注意这里有一个和系统不一致的行为：当 titleLabel 为多行时，系统的 sizeThatFits: 计算结果固定是单行的，所以当 JXButtonImagePositionLeft 并且titleLabel 多行的情况下，JXButton 计算的结果与系统不一致
...
- (void)layoutSubviews {
    ...
    if (self.imagePosition == JXButtonImagePositionTop || self.imagePosition == JXButtonImagePositionBottom) {
        ...
        if (self.imagePosition == JXButtonImagePositionTop) {
    ...
    } else if (self.imagePosition == JXButtonImagePositionLeft || self.imagePosition == JXButtonImagePositionRight) {
        ...
        if (self.imagePosition == JXButtonImagePositionLeft) {
            switch (self.contentHorizontalAlignment) {
                case UIControlContentHorizontalAlignmentLeft:
                    imageFrame = isImageViewShowing ? CGRectSetX(imageFrame, contentEdgeInsets.left + self.imageEdgeInsets.left) : imageFrame;
...
- (void)setImagePosition:(JXButtonImagePosition)imagePosition {
