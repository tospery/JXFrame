...
/// 控制图片在UIButton里的位置，默认为JXButtonImagePositionLeft
typedef NS_ENUM(NSUInteger, JXButtonImagePosition) {
    JXButtonImagePositionTop,             // imageView在titleLabel上面
    JXButtonImagePositionLeft,            // imageView在titleLabel左边
    JXButtonImagePositionBottom,          // imageView在titleLabel下面
    JXButtonImagePositionRight,           // imageView在titleLabel右边
};
 ...
 *  @warning JXButton 重新定义了 UIButton.titleEdgeInests、imageEdgeInsets、contentEdgeInsets 这三者的布局逻辑，sizeThatFits: 里会把 titleEdgeInests 和 imageEdgeInsets 也考虑在内（UIButton 不会），以使这三个接口的使用更符合直觉。
 */
...
/**
 * 设置按钮里图标和文字的相对位置，默认为JXButtonImagePositionLeft<br/>
 * 可配合imageEdgeInsets、titleEdgeInsets、contentHorizontalAlignment、contentVerticalAlignment使用
 */
@property(nonatomic, assign) JXButtonImagePosition imagePosition;
