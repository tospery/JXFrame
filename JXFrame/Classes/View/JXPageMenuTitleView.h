...
@class JXPageMenuTitleView;

@protocol JXCategoryTitleViewDataSource <NSObject>
@optional
// 如果将JXPageMenuTitleView嵌套进UITableView的cell，每次重用的时候，JXPageMenuTitleView进行reloadData时，会重新计算所有的title宽度。所以该应用场景，需要UITableView的item缓存titles的文字宽度，再通过该代理方法返回给JXPageMenuTitleView。
// 如果实现了该方法就以该方法返回的宽度为准，不触发内部默认的文字宽度计算。
- (CGFloat)categoryTitleView:(JXPageMenuTitleView *)titleView widthForTitle:(NSString *)title;
@end
...
@property (nonatomic, assign) JXPageMenuTitleLabelAnchorPointStyle titleLabelAnchorPointStyle;  //titleLabel锚点位置，用于调整titleLabel缩放时的基准位置。默认为：JXPageMenuTitleLabelAnchorPointStyleCenter
@end
