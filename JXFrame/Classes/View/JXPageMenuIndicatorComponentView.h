//
//  JXPageMenuIndicatorComponentView.h
//  Pods
//
...
/**
 指示器的位置。底部或者顶部
 */
@property (nonatomic, assign) JXPageMenuComponentPosition componentPosition;

/**
 默认JXPageAutomaticDimension（与cell的宽度相等）。内部通过`- (CGFloat)indicatorWidthValue:(CGRect)cellFrame`方法获取实际的值
 */
...
/**
 默认JXPageAutomaticDimension （等于indicatorHeight/2）。内部通过`- (CGFloat)indicatorCornerRadiusValue:(CGRect)cellFrame`方法获取实际的值
 */
...
/**
 手势滚动、点击切换的时候，如果允许滚动，分为简单滚动和复杂滚动。默认为：JXPageMenuIndicatorScrollStyleSimple
 目前仅JXPageMenuIndicatorLineView、JXPageMenuIndicatorDotLineView支持，其他子类暂不支持。
 */
@property (nonatomic, assign) JXPageMenuIndicatorScrollStyle scrollStyle;
